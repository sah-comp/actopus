...
class Controller_Setting extends Controller_Scaffold
{
    ...
    public function index($page = 1, $limit = self::LIMIT, $layout = self::LAYOUT, $order = 0, $dir = 0)
    {
        ...
        session_start();
        if (! $this->auth()) {
            $this->redirect(sprintf('/login/?goto=%s', urlencode('/setting/index/')));
        }
        ...
        $this->view->shouldRevalidateCards = R::count('card', "revalidate = 1");
    ...
    public function rbac()
    {
        ...
        session_start();
        if (! $this->auth()) {
            $this->redirect(sprintf('/login/?goto=%s', urlencode('/setting/rbac/')));
        }
    ...
    public function language($iso)
    {
        ...
        session_start();
        if (! $this->auth()) {
            $this->redirect(sprintf('/login/?goto=%s', urlencode('/home/')));
        }
